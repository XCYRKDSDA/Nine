    ...
    where TValue : struct
{
    ...
    protected override TValue LinearInterpolate(in TValue p0, in TValue p1, float k) =>
        GenericMathHelper<TValue>.Add(
            GenericMathHelper<TValue>.Mul(in p0, 1 - k),
            GenericMathHelper<TValue>.Mul(in p1, k)
        );

    protected override TValue SmoothInterpolate(
        in TValue p0,
        in TValue m0,
        in TValue p1,
        in TValue m1,
    ...
    )
    {
        ...
        return GenericMathHelper<TValue>.Add(
            GenericMathHelper<TValue>.Add(
                GenericMathHelper<TValue>.Mul(in p0, h00),
                GenericMathHelper<TValue>.Mul(in m0, h10)
            ),
            GenericMathHelper<TValue>.Add(
                GenericMathHelper<TValue>.Mul(in p1, h01),
