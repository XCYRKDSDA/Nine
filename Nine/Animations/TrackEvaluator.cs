...
public static class TrackEvaluator
{
                                                    ...
                                                    ICurve<float>? tweener, float k) where ValueT : struct
    {
        ...
        // 混合两条曲线的输出, 并赋值
        var result = GenericMathHelper<ValueT>.Add(GenericMathHelper<ValueT>.Mul(in value1, 1 - k),
    ...
    private delegate void EvaluateAndSetMethod<ObjectT>(ref ObjectT obj, IProperty<ObjectT> property, ICurve? curve,
                                                        float t);
    ...
    private static readonly MethodInfo EvaluateAndSet_Wrapper_Info =
        typeof(TrackEvaluator).GetMethod("EvaluateAndSet_Wrapper", BindingFlags.NonPublic | BindingFlags.Static)!;

    private static readonly MethodInfo TweenAndSet_Wrapper_Info =
        typeof(TrackEvaluator).GetMethod("TweenAndSet_Wrapper", BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    private static (Delegate, Delegate) GetEvaluationMethodsFromCache(Type objectType, Type valueType)
    {
        ...
        var evaluateAndSetHandler =
            Delegate.CreateDelegate(typeof(EvaluateAndSetMethod<>).MakeGenericType(objectType),
                                    evaluateAndSetHandlerInfo);
        var tweenAndSetHandlerInfo = TweenAndSet_Wrapper_Info.MakeGenericMethod(objectType, valueType)!;
        var tweenAndSetHandler =
            Delegate.CreateDelegate(typeof(TweenAndSetMethod<>).MakeGenericType(objectType), tweenAndSetHandlerInfo);
    ...
    public static void EvaluateAndSet<ObjectT>(ref ObjectT obj, IProperty<ObjectT> property, Type valueType,
    ...
    public static void TweenAndSet<ObjectT>(ref ObjectT obj, IProperty<ObjectT> property, Type valueType,
                                            ICurve? curve1, float t1, ICurve? curve2, float t2, ICurve<float>? tweener,
                                            float k)
    {
    ...
    public static void TweenAndSet<ObjectT>(ref ObjectT obj,
                                            TrackCollection<ObjectT> tracks1, float t1,
                                            TrackCollection<ObjectT> tracks2, float t2,
                                            ICurve<float>? tweener, float k)
    {
        // 遍历两个轨道集合关注的属性的交集
