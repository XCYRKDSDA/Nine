...
public class KeyFrameCurveLoader<TValue, TGradient, TCurve> : ICurveLoader<TValue>
    where TValue : struct
    where TGradient : struct
    where TCurve : KeyFrameCurve<TValue, TGradient>, new()
{
    public KeyFrameCurveLoader(
    ...
    {
        var jsonKeyFrames =
            jsonElement.Deserialize<JsonKeyFrame[]>(_jsonSerializerOptions)
