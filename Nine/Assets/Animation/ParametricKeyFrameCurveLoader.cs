...
public class ParametricKeyFrameCurveLoader<TValue, TGradient, TCurve>
    : IParametricCurveLoader<TValue>
    where TValue : struct
    where TGradient : struct
    where TCurve : KeyFrameCurve<TValue, TGradient>, new()
{
    public ParametricKeyFrameCurveLoader(
        JsonConverter<IParametric<TValue>>? valueConverter,
    ...
    )
    {
        ...
        if (valueConverter is not null)
            _jsonSerializerOptions.Converters.Add(valueConverter);
        if (gradientConverter is not null)
            _jsonSerializerOptions.Converters.Add(gradientConverter);
    }
    ...
    {
        var jsonKeyFrames =
            jsonElement.Deserialize<JsonParametricKeyFrame[]>(_jsonSerializerOptions)
        ...
        {
            curve.KeyFrames.Add(
                new(jsonKeyFrame.Time, jsonKeyFrame.Value, jsonKeyFrame.Type, jsonKeyFrame.Gradient)
            );
        }
